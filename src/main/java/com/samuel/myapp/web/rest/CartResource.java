...
import com.samuel.myapp.service.CartService;
import com.samuel.myapp.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.samuel.myapp.domain.Cart}.
 */
@RestController
@RequestMapping("/api")
public class CartResource {
     ...
     */
    @PutMapping("/carts/{id}")
    public ResponseEntity<Cart> updateCart(@PathVariable(value = "id", required = false) final String id, @RequestBody Cart cart)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, cart.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Cart> partialUpdateCart(@PathVariable(value = "id", required = false) final String id, @RequestBody Cart cart)
        throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, cart.getId()));
    }
